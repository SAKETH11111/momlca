...
`train_rf_baseline`, `predict_rf`, `save_rf_model`, and `load_rf_model` expose the sklearn `RandomForestRegressor` workflow directly while preserving feature-name metadata for interpretability. When multi-target labels are sparse, the helper intentionally falls back to the per-property wrapper so partially labeled PFASBench rows are kept instead of discarded. `train_xgb_baseline`, `predict_xgb`, `save_xgb_model`, and `load_xgb_model` provide the compact XGBoost interface, with native `XGBRegressor` behavior for single-target use and the project wrapper retained for sparse multi-target comparison runs.
