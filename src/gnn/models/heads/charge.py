"""Charge prediction heads for node-level regression."""
...
@register_head("charge")
class ChargeHead(nn.Module):
    """MLP head for per-atom charge prediction with uncertainty output."""
    ...
    def __init__(
        ...
        self.charge_layer = nn.Linear(previous_dim, 1)
        self.log_variance_layer = nn.Linear(previous_dim, 1)

    def forward(self, node_features: torch.Tensor) -> dict[str, torch.Tensor]:
        """Project node embeddings to per-atom charge predictions and uncertainty."""
        hidden = self.hidden_layers(node_features)
        charges = self.charge_layer(hidden).squeeze(-1)
        log_variance = self.log_variance_layer(hidden).squeeze(-1)
        return {
            "predictions": charges,
